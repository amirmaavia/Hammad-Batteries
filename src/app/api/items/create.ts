...
export async function POST(req: Request) {
  try {
    ...
    const { name, brand, subCategory, description, seoTitle, seoDescription, seoKeywords, defaultPrice, originalPrice, stock, image, images, video, videoId, featured, imageFit } = body;
    ...
    return NextResponse.json(
      {
        success: true,
        data: { _id: id, name, brand, subCategory, description: description || "", seoTitle: seoTitle || "", seoDescription: seoDescription || "", seoKeywords: seoKeywords || "", defaultPrice, originalPrice, stock, image: primaryImage, images: galleryImages, video: "", videoId: savedVideoId, featured: Boolean(featured), imageFit: imageFit || "fit" },
      },
      { status: 201 }
    );
  } catch (error) {
